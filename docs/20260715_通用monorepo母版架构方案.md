...
3. **地基齐全** —— 任何 web 业务几乎必用的能力(路由、分页、定时任务、生产日志、配置服务、请求追踪、迁移建表)开箱即用(见"九、基座能力")。
...
- 常用地基: 前端路由 + SPA 深链兜底、进程内定时任务、列表分页、生产级日志、配置服务、请求追踪(request-id)、启动即迁移建表(见"九")。
...
| 25 | 启动建表 | `lifespan` 启动跑 `alembic upgrade head`(不用 `create_all`) | `create_all` 只建缺失表、不 alter 已有表, 团队后续加字段会"默认即漏迁移"; 统一走迁移让开发/生产与迁移历史一致 | env.py 在 online 模式自行 `asyncio.run`, 故 lifespan 用 `asyncio.to_thread` 调用避免嵌套; `alembic.ini` 须保持 ASCII(configparser 按 OS locale 读, 中文注释会在 zh-CN Windows 崩) |
| 26 | 请求追踪 | request-id 中间件 + `structlog.contextvars` | 排障刚需: 把一次请求的所有日志串起来; 入站 `X-Request-ID` 透传否则生成, 回写响应头 | 只做最小追踪; 接 OpenTelemetry / 分布式 trace 在此扩展(seam) |
...
以下几项是"任何 web 业务几乎必用"的地基, 母版已按硬约束(只用 GET/POST、完整三层、同源单进程、质量三闸门)落地, 团队照抄扩展即可。每项都给了扩展的 seam, 不预置具体业务。
...
```mermaid
flowchart TD
  Root["根布局 RootLayout: 顶栏 + 资源卡片 + 导航"]
  Root --> Home["index 主页: 价值 / 架构 / 能力一览"]
  Root --> Items["items 路由: Item CRUD 示例, 含分页"]
  Root --> NotFound["通配 星号: 404 页"]
```
...
sequenceDiagram
  ...
  U->>L: startup
  L->>L: alembic upgrade head 迁移建表
  L->>S: register_jobs 后 scheduler.start
  S-->>J: 按触发器周期调度
  U->>L: shutdown
  L->>S: scheduler.shutdown 不等待
```
...
### 9.6 请求追踪(request-id 贯穿日志)

排障刚需: 把"一次请求产生的所有日志"串起来。`core/middleware.py` 的 `request_id_middleware`:

- 入站带 `X-Request-ID` 则**透传**(便于跨服务串联), 否则生成一个(`uuid4`)。
- 用 `structlog.contextvars.bind_contextvars` 绑定到上下文; `core/logging` 的 `merge_contextvars` 会把它自动注入这次请求的**每一条日志**, 各处无需手传。
- 响应回写 `X-Request-ID`, 前端 / 调用方可拿来对账、报障。

在 `create_app` 里 `app.middleware("http")(request_id_middleware)` 注册(后添加者最外层, 包住整条链)。母版只做这一层最小追踪; 接 OpenTelemetry / 分布式 trace 时在此扩展(seam)。

### 9.7 建表走迁移(不用 create_all)

启动时 `lifespan` 跑 `alembic upgrade head`(`db/migrate.py`), **不用 `create_all`**:

- `create_all` 只建"缺失的表", **不 alter 已有表**。团队后续用 Alembic 加字段却还留着 `create_all`, 会误以为迁移生效而实际漏迁移(默认即错)。统一走迁移, 让开发 / 生产与迁移历史始终一致。
- 初始迁移 `migrations/versions/*_initial_item_table.py` 已随母版提交(`alembic revision --autogenerate` 生成)。团队加实体后: 在 `db/base.py` import 一次 -> `alembic revision --autogenerate -m "..."` 生成迁移 -> 启动自动 `upgrade head`。
- **落地要点**: env.py 在 online 模式自行 `asyncio.run`, 故 lifespan 用 `asyncio.to_thread(run_migrations_to_head)` 调用, 避免与事件循环嵌套。`alembic.ini` **须保持 ASCII** —— configparser 按 OS locale 编码读它, 中文注释会在 zh-CN Windows(GBK)启动时崩。
