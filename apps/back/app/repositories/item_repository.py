...
- 只做 add / get / list / count / delete 并 flush(拿自增主键), 不 commit ——
...
class ItemRepository:
    ...
    async def list(self, limit: int, offset: int) -> Sequence[Item]:
        result = await self._session.exec(select(Item).order_by(col(Item.id)).limit(limit).offset(offset))
        return result.all()

    async def count(self) -> int:
        result = await self._session.exec(select(func.count()).select_from(Item))
